...
def parse_args():
    ...
    parser.add_argument("-base_app_path", action="store", dest="base_app_path", required=True,
                   help="Path to the base version APK")
    parser.add_argument("-new_app_dir", action="store", dest="new_app_dir", required=True,
                    help="Directory containing new version APKs")
...
def main():
    ...
    # 检查基准版本和新版本目录
    if not os.path.exists(opts.base_app_path):
        print(f"Base APK {opts.base_app_path} does not exist.")
        return
    if not os.path.exists(opts.new_app_dir):
        print(f"New APK directory {opts.new_app_dir} does not exist.")
        return
        
    # 获取新版本APK列表
    new_apps = [f for f in os.listdir(opts.new_app_dir) if f.endswith('.apk')]
    if not new_apps:
        print(f"No APK files found in {opts.new_app_dir}")
        return
    
    # 构造 app_path 列表
    opts.app_path = [opts.base_app_path]  # 基准版本
    for app in new_apps[:15]:  # 最多添加15个新版本
        opts.app_path.append(os.path.join(opts.new_app_dir, app))
    
    # 构造对应数量的 append_device 列表
    opts.append_device = []
    for i in range(len(opts.app_path)):
        opts.append_device.append(f"emulator-{5554 + i*2}")
